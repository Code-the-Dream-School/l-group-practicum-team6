...
describe('auth api', () => {
  it('getUser calls correct endpoint', () => {
    ...
    expect(mockedApiFetch).toHaveBeenCalledWith(API_ROUTES.CURRENT_USER);
  });
...
describe('users api', () => {
  ...
  it('changePassword patches password data', () => {
    ...
    expect(mockedApiFetch).toHaveBeenCalledWith(API_ROUTES.CURRENT_USER_PASSWORD, {
      method: 'PATCH',
      body: JSON.stringify({
        currentPassword: 'oldpass',
        newPassword: 'newpass',
  ...
  it('getSavedVisuals calls saved visuals endpoint', () => {
    ...
    expect(mockedApiFetch).toHaveBeenCalledWith(API_ROUTES.CURRENT_USER_VISUALS);
  });
...
describe('images api', () => {
  ...
  it('deleteAvatar deletes avatar', () => {
    ...
    expect(mockedApiFetch).toHaveBeenCalledWith(API_ROUTES.CURRENT_USER_IMAGE, {
      method: 'DELETE',
    });
  });
