...
describe('connectDB', () => {
  ...
  it('connects successfully and logs a success message', async () => {
    ...
    expect(mockedConnect).toHaveBeenCalledWith('mongodb://127.0.0.1:27017/test-db');
  ...
  it('logs an error and exits the process when connection fails', async () => {
    ...
    await expect(connectDB('bad-uri')).rejects.toThrow('process.exit called with code 1');

    expect(mockedConnect).toHaveBeenCalledWith('bad-uri');
    expect(consoleErrorSpy).toHaveBeenCalledWith('Failed to connect to MongoDB:', dbError);
